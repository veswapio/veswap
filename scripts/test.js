...
const totalArr = result0.concat(result1).concat(result2)
...
for (let i = 0; i < totalArr.length; i++) {
  ...
  for (let k = 0; k < liquidityMapKeys.length; k++) {
    ...
    if (liquidityAccumulatedMap[liquidityMapKeys[k]] === undefined) {
      ...
      pointsArray[liquidityMapKeys[k]] = pointsArray[liquidityMapKeys[k]] + parseInt(liquidityAccumulatedMap[liquidityMapKeys[k]])
    }
  }
}

const sortedPointsArray = Object.entries(pointsArray).sort((a, b) => b[1] - a[1]).map(item => [item[0], parseInt(item[1])])

fs.writeFileSync("./points.json", JSON.stringify(sortedPointsArray, null, 2));
// console.log(aaa0.concat(aaa).length)
